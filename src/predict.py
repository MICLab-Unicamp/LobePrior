...
from src.utils.general import register_single, teste_pickle_by_image, process_images
from src.utils.general import unified_img_reading, convert_to_nifti, remove_directories_if_exist, collect_images_verbose
from src.utils.general import analyze_registration_quality, find_best_registration
from src.utils.transform3D import CTHUClip
from src.predict_decoders import LoberModule
from src.predict_normal import LoberModuleNormal

HOME = os.getenv("HOME")
TEMP_IMAGES = 'temp_images'
RAW_DATA_FOLDER = 'src/raw_images' #os.path.join(HOME, 'raw_images')
...
def run(args):
	...
	for image_original_path in all_images:
		...
		if modo_normal:
			...
			pre_trained_model_path = 'src/weights/LightningLobes_no_template.ckpt'

			if not os.path.isfile(pre_trained_model_path):
				raise FileNotFoundError(f"Checkpoint not found: {pre_trained_model_path}")
			...
			pre_trained_model_path = 'src/weights/LightningLobes.ckpt'
			...
			try:
				...
				raise RuntimeError(f"Error while loading the checkpoint: {e}")
...
def main():
	...
	parser.add_argument("--output", "-o", default="outputs", help="Directory to store the final segmentation.", type=str)
	parser.add_argument("--nworkers", "-nw", default=mp.cpu_count() // 2,  help="Number of workers", type=int)
	parser.add_argument("--normal", "-n", action="store_true", help="Do not use Prior Information.")
	parser.add_argument("--delete", "-d", action="store_true", help="Delete temporary files.")
	parser.add_argument("--pool", "-p", action="store_true", help="Parallel processing.")
